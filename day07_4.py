# 생성자 : 클래스는 초보자 위해서 만듦, 근데 얘가 제대로 사용을 못함 
class 회사프로그램:
    n1 = 0      # 클래스 안에 있는 변수 == 멤버변수
    n2 = 0
    # 메서드 ; 클래스 안에 있는 함수(매개변수 가장 앞에 self하는 매개 변수를 만들어놓아야함)
    def AbsSum(self, n1, n2):       # self : 메서드 고정 매개변수로 사용할 때 채우지 않음(사용X)
        ...
        result = n1+n2
        return result
def Last(self):         # 사실상 개매변수가 없는 메서드 (self)는 함수가 아니라 메서드라는 표시로 일단은 생각
...
a = 회사프로그램(3, 5)
# 생성자라는 문법을 추가해서 사전 작없을 사람이 하지 않아도 자동사용되게끔 처리
a.Last()
# 클래스이름 옆에 있는 () __init__를 사용하는 코드
b = 회사프로그램(5, 2)
b.Last()
